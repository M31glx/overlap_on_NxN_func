...
% Date: 21/11/10
%
% adapted by J. Faskowitz so that the C1 and C2 that the function takes in
% are NodeXCommuinity binary mats, which are converted to cell arrays that
% the original function wanted
% 
function [NXY] = gnmi(C1, C2, N)

    % convert to cell array
    C1_tmp = cell(size(C1,2),1) ;
    C2_tmp = cell(size(C2,2),1) ;
    
    for idx = 1:size(C1,2)
       C1_tmp{idx} = find(C1(:,idx)) ; 
    end
    for idx = 1:size(C2,2)
       C2_tmp{idx} = find(C2(:,idx)) ; 
    end
    ...
    % H(Xk|Yl)
    nbtestc1 = zeros(length(C1_2),1);
    minHXY = Inf(length(C1_2),1);
    nbtestc2 = zeros(length(C2_2),1);
    minHYX = Inf(length(C2_2),1);
    for c1=1:length(C1_2)
        lc1 = length(C1_2{c1});
        for c2=1:length(C2_2)
            lc2 = length(C2_2{c2});
            %l12 = length(intersect(C1{c1},C2{c2}));
            l12 = intersect_size(C1_2{c1},C2_2{c2});
            p11 = l12/N;
            p10 = (lc1 - l12)/N;
            p01 = (lc2 - l12)/N;
            p00 = (N - (lc1 + lc2 - l12))/N;
